...
static void find_obj(u8 *argv0) {
  ...
  u8 *Polar_path = getenv("Polar_PATH");
  u8 *slash, *tmp;

  if (Polar_path) {

    tmp = alloc_printf("%s/dfsan/dfsan_abilist.txt", Polar_path);
  ...
  if (slash) {
    ...
    *slash = '/';
    ...
    tmp = alloc_printf("%s/dfsan_abilist.txt", dir);
    if (!access(tmp, R_OK)) {
      obj_path = dir;
      ck_free(tmp);
      return;
  ...
  FATAL("Unable to find 'dfsan_abilist.txt'");
}

static void check_type(char *name) {
  if (!strcmp(name, "Polar-clang++")) {
    is_cxx = 1;
  }
}
...
static void add_polar_runtime()
{
  cc_params[cc_par_cnt++] = "-fsanitize=dataflow";
  cc_params[cc_par_cnt++] = alloc_printf("-fsanitize-blacklist=%s/dfsan/dfsan_abilist.txt", obj_path);
  cc_params[cc_par_cnt++] = "-includePolar/Polar_label.h";
  cc_params[cc_par_cnt++] = "-lPolar_dfsan";
}
...
static void edit_params(u32 argc, char **argv) {
  ...
  if (is_cxx) {
    u8 *alt_cxx = getenv("POLAR_CXX");
    cc_params[0] = alt_cxx ? alt_cxx : (u8 *)"clang++";
  } else {
    u8 *alt_cc = getenv("POLAR_CC");
  ...
  while (--argc) {
    ...
    if (!strcmp(cur, "-fsanitize=address"))
      FATAL("ASAN is not supported by your compiler and meaningless in this stage!");

    if(!strcmp(cur, "-fsanitize=memory"))
      FATAL("MSAN is not supported by your compiler and meaningless in this stage!");
